...
export function ConnectWalletProvider({
    ...
    const [connectedWallet, setConnectedWallet] = useState<CIP30API | null>(
        null,
    );
    ...
    const [connectedWalletProvider, setConnectedWalletProvider] = useState<
        any | null
    >(null);
    ...
    const deriveStakeAddress = (
        hexStakeAddressBytes: string,
        isTestnet: boolean,
    ): string => {
        try {
            ...
            try {
                ...
                console.warn(
                    'Could not parse as Address, trying as RewardAddress:',
                    error,
                );

                try {
                    const rewardAddress =
                        CardanoWasm.RewardAddress.from_bytes(addressBytes);
                    const bech32Address = rewardAddress
                        .to_address()
                        .to_bech32();
                    return bech32Address;
                } catch (error2) {
                    console.warn(
                        'Could not parse as RewardAddress either:',
                        error2,
                    );
                    const prefix = isTestnet ? 'stake_test1' : 'stake1';
                    const simplifiedAddress = `${prefix}${hexStakeAddressBytes}`;
                    console.log(
                        'Using simplified stake address (fallback):',
                        simplifiedAddress,
                    );
    ...
    const extractSignature = async (message: string) => {
        try {
            ...
            if (userAddress.startsWith('addr')) {
                addressToSignWith =
                    CardanoWasm.Address.from_bech32(userAddress).to_hex();
            } else {
    ...
    const connectWallet = async (walletName: string) => {
        try {
            ...
            if (!wallet)
                throw new Error(
                    t('wallet.connect.errors.walletNotFound', { walletName }),
                );
            ...
            if (!allowedNetworks.includes(walletNetworkId)) {
                const expectedNetwork =
                    environment === 'production'
                        ...
                        : 'Preview/Testnet';
                throw new Error(
                    t('wallet.connect.errors.wrongNetwork', {
            ...
            try {
                ...
                if (rewardAddresses && rewardAddresses.length > 0) {
                    ...
                    derivedStakeAddress = deriveStakeAddress(
                        stakeKeyHex,
                        isTestnet,
                    );
            ...
            setError(
                err instanceof Error
                    ? err.message
                    : t('wallet.connect.errors.connectionFailed'),
            );
        } finally {
