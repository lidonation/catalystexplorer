...
const RelatedIdeascaleProfilesList: React.FC<IdeascaleProfilesListProps> = ({ideascaleProfiles, ...props}) => {
    ...
    return (
        <AnimatePresence>
            <div className="grid w-full grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5" {...props}>
                {!!ideascaleProfiles?.data && ideascaleProfiles?.data.map((ideascaleProfile, index) => (
                    <motion.div
                        key={index}
                        ...
                        transition={{duration: 0.4, ease: 'easeIn'}}
                        className='h-full border-border-dark-on-dark rounded-lg border-2'
                    >
                ...
                {(!!ideascaleProfiles?.data && ideascaleProfiles?.total > ideascaleProfiles?.per_page) && (
                    <div className=''>
                        <Link
                            href='#'
