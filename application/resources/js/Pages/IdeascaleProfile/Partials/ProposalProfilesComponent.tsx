...
interface ProposalProfileUsers extends Record<string, unknown> {
    users: (App.DataTransferObjects.IdeascaleProfileData | any)[];
    onUserClick?: (user: App.DataTransferObjects.IdeascaleProfileData | any) => void;
    className?: string;
    toolTipProps: any;
}
...
export default function ProposalProfiles({
    ...
    const handleGenerateLink = () => {
        ...
        const ideascaleUsers = users?.filter(user =>
            'profile_type' in user && (user.profile_type === 'App\\Models\\IdeascaleProfile' || user.profile_type === 'IdeascaleProfile')
        ) || [];
        ...
        // For now, redirect to ideascale profiles search with all user names
