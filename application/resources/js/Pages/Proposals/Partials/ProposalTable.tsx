...
interface ColumnConfig {
  ...
  label: string;
  sortable?: boolean;
  renderCell: (proposal: ProposalData, helpers: TableHelpers) => React.ReactNode;
}
...
const ProposalTable: React.FC<ProposalTableProps> = ({ proposals }) => {
  ...
  const [selectedUserMap, setSelectedUserMap] = useState<Record<string, IdeascaleProfileData | null>>({});
  ...
  const getRowHelpers = useCallback((proposalHash: string): TableHelpers => {
    return {
      selectedUser: selectedUserMap[proposalHash] ?? null,
      handleUserClick: (user: IdeascaleProfileData) => {
        setSelectedUserMap(prev => ({ ...prev, [proposalHash]: user }));
      },
      noSelectedUser: () => {
        setSelectedUserMap(prev => ({ ...prev, [proposalHash]: null }));
      }
    };
  }, [selectedUserMap]);
