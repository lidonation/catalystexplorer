...
const MetaData = ({ nft, isOwner }: MetaDataProps) => {
    ...
    const saveStruckStates = (
        storageKey: string,
        struckStates: Record<string, boolean>,
    ) => {
        try {
            localStorage.setItem(
                `${storageKey}_struck`,
                JSON.stringify(struckStates),
            );
        } catch (e) {
                ...
                if (nft.metadata) {
                    ...
                    const keyMappings = {
                        campaignName: [
                            ...
                            'campaign_name',
                            'projectCatalystCampaignName',
                        ],
                        projectNumber: [
                            'Funded Project Number',
                            'funded_project_number',
                            'fundedProjectNumber',
                        ],
                        projectTitle: [
                            'Project Title',
        ...
        return (
            <div className="border-dark border-b pb-4">
                <div className="grid grid-cols-[200px_1fr] items-start">
                    <div className="text-dark text-sm">{label}</div>
                    <div className="flex items-center gap-2">
                        <span
                            className={`text-sm ${struckFields[fieldKey] ? 'text-dark line-through' : ''}`}
                        >
                            {displayValue}
                        </span>
                        {isOwner && (
                            <Button
                                ...
                                className={`border-none bg-transparent p-0 text-sm ${isFieldUpdating ? 'cursor-not-allowed opacity-50' : ''}`}
                                ariaLabel={
        ...
        return (
            <div className="flex flex-col items-center justify-center py-12">
                <div className="mb-4 text-center">
                    <span className="text-dark inline-block rounded-lg px-4">
        ...
        <div className="bg-background rounded-lg p-6">
            ...
            {isMetadataAvailable ? (
                <div className="space-y-6">
                    {renderReadOnlyField(
                        t('projectCatalyst'),
                        displayValues.campaignName,
                    )}
                    {renderReadOnlyField(
                        t('fundProject'),
                        displayValues.projectNumber,
                    )}
                    {renderReadOnlyField(
                        t('projectTitle'),
                        displayValues.projectTitle,
                    )}
                    {renderEditableField(
                        t('yesVotes'),
                        'yesVotes',
                        displayValues.yesVotes,
                    )}
                    {renderEditableField(
                        t('noVotes'),
                        'noVotes',
                        displayValues.noVotes,
                    )}
            ...
            {isOwner && isMetadataAvailable && (
                <div className="mt-8 flex justify-center">
                    <Paragraph className="text-dark max-w-lg text-center text-xs italic">
