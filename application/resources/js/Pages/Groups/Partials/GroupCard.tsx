...
const CampaignCard: React.FC<GroupCardProps> = ({ group }) => {
  ...
  return (
    <Card>
      <div>
        <Paragraph className='border-b border-dark pb-3 pt-2' children={t('bio')}/>
        <Paragraph className="text-content-dark opacity-80 mb-4 line-clamp-3 pt-4" children={group?.bio} />
        <div className='flex flex-col gap-2'>
            <div className='flex justify-between border-b pb-4 border-dark'>
              <div>
                  ...
                  <Paragraph className='text-dark' children={t('groups.totalRequested')}/>
              </div>
              <div>
                <Paragraph className='text-xl font-bold' children={group?.proposals_count ?? 0}/>
                <Paragraph className='text-dark' children={t('proposals.proposals')}/>
              </div>
            </div>
        </div>
        ...
        <Paragraph className='pt-4 text-dark border-b pb-4' children={t('groups.awardedVsRequested')}/>

        <div
            className={`grid ${noAwardedFunds || allAwardedFunds ? 'grid-cols-2 pt-4' : 'grid-cols-1'} mt-4 gap-4`}
        >
            {(group?.amount_awarded_ada || noAwardedFunds) && (
                <div>
                    <GroupFundingPercentages
        ...
        <Paragraph className='border-b pt-4 pb-4 mb-4 text-dark' children={t('groups.receivedVsAwarded')}/>
      </div>
    </Card>
  );
};
