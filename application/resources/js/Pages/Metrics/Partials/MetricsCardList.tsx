...
const MetricCardList: React.FC<MetricProps> = ({
    metrics,
    sortBy,
    sortOrder,
    columns,
    ...
    const sortedMetrics =
        ...
        [...metrics].sort((a, b) => {
            ...
            let valueA =
                a[sortBy] ?? (typeof a[sortBy] === 'number' ? 0 : new Date(0));
            let valueB =
                b[sortBy] ?? (typeof b[sortBy] === 'number' ? 0 : new Date(0));
    ...
    const columnClass =
        columns === MetricEnum.TWO_COLUMNS ? 'grid-cols-2' : 'grid-cols-3';
