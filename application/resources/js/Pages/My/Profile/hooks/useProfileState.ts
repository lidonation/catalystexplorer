...
type ProfileAction =
    ...
    | {
          ...
          payload: Partial<ProfileState['modalConfig']>;
      }
...
export default function useProfileState() {
    ...
    return {
        ...
        setIsPublic: (value: boolean) =>
            dispatch({ type: 'SET_PUBLIC', payload: value }),
        setCurrentModal: (type: ModalType) =>
            dispatch({ type: 'SET_CURRENT_MODAL', payload: type }),
        setModalConfig: (config: Partial<ProfileState['modalConfig']>) =>
            dispatch({ type: 'SET_MODAL_CONFIG', payload: config }),
        setPhotoPreview: (url: string | null) =>
            dispatch({ type: 'SET_PHOTO_PREVIEW', payload: url }),
        setPhotoUploading: (value: boolean) =>
            dispatch({ type: 'SET_PHOTO_UPLOADING', payload: value }),
        setPhotoError: (error: string) =>
            dispatch({ type: 'SET_PHOTO_ERROR', payload: error }),
        setCopySuccess: (value: boolean) =>
            dispatch({ type: 'SET_COPY_SUCCESS', payload: value }),
        closeModal: () => dispatch({ type: 'CLOSE_MODAL' }),
        openProfileFieldModal: (
            config: Partial<ProfileState['modalConfig']>,
        ) => {
            dispatch({ type: 'SET_MODAL_CONFIG', payload: config });
            dispatch({
                type: 'SET_CURRENT_MODAL',
                payload: ModalType.PROFILE_FIELD,
            });
        },
        openSocialProfilesModal: () =>
            dispatch({
                type: 'SET_CURRENT_MODAL',
                payload: ModalType.SOCIAL_PROFILES,
            }),
        openPasswordModal: () =>
            dispatch({
                type: 'SET_CURRENT_MODAL',
                payload: ModalType.PASSWORD,
            }),
    };
}
