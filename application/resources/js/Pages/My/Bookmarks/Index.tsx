...
const Index: React.FC<IndexProps> = ({
    ...
    const renderEmptyState = () => (
        <div className="flex min-h-[50vh] flex-col items-center justify-center px-4 text-center">
            <Title className="mb-4 text-2xl font-bold text-gray-600">
                {t('noBookmarks')}
            </Title>
            <Paragraph className="max-w-md text-gray-500">
                {t('noBookmarksYet')}
            </Paragraph>
        </div>
    );
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            const currentSection = Object.entries(sectionsRef.current).find(
                ([, ref]) => {
                    if (!ref) return false;
            ...
            if (currentSection) {
                ...
                const type =
                    sectionTypes[sectionKey as keyof typeof sectionTypes];
                setActiveType(type);
            }
        };
    ...
    const scrollToSection = (type: string) => {
        const sectionKey = Object.keys(sectionTypes).find(
            (key) => sectionTypes[key as keyof typeof sectionTypes] === type,
        );
        if (sectionKey && sectionsRef.current[sectionKey]) {
            sectionsRef.current[sectionKey]?.scrollIntoView({
                behavior: 'smooth',
            });
        }
    };
