...
export default function TabNavigation({
    ...
    tabClassName = 'whitespace-nowrap text-sm md:text-base px-1 md:px-2 py-1 group flex items-center gap-1 outline-hidden transition-colors hover:text-gray-700 dark:hover:text-gray-400 font-bold text-gray-500 dark:text-gray-500',
    ...
    const centerTabInView = useCallback(
        (tabIndex: number) => {
            if (
                !centerTabs ||
                !navRef.current ||
            ...
            const scrollContainer = navRef.current.closest(
            ...
            if (isTabFullyVisible) {
                const prevTabRect =
                    tabIndex > 0
                        ? tabRefs.current[tabIndex - 1]?.getBoundingClientRect()
                        : null;
                const nextTabRect =
                ...
                const hasInvisibleNeighbor =
                    (prevTabRect &&
                        prevTabRect.left < containerRect.left - 5) ||
                    (nextTabRect &&
                        nextTabRect.right > containerRect.right + 5);
    ...
    useEffect(() => {
        ...
        if (activeTabIndex !== -1) {
            const timer = setTimeout(
                () => centerTabInView(activeTabIndex),
                100,
            );
