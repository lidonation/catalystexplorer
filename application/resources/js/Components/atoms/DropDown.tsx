import { useEffect, useRef, ReactNode, Component } from 'react';
...
export default function DropDown({
  ...
  useEffect(() => {
    if (mode === 'dropdown' && triggerRef?.current && popupRef.current && isOpen) {
      ...
      if (position) {
        ...
        if (position.right !== undefined) popupRef.current.style.right = typeof position.right === 'number' ? `${position.right}px` : position.right;
        if (position.bottom !== undefined) popupRef.current.style.bottom = typeof position.bottom === 'number' ? `${position.bottom}px` : position.bottom;
      }
    }
  ...
  if (mode === 'modal') {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div
          ref={popupRef}
          className={`bg-white rounded-lg shadow-xl p-6 max-w-md relative ${className}`}
        >
          <CloseButton />

          {title && <Title level="2" className="mb-4">{title}</Title>}
 ...
 return (
    <div className="fixed inset-0 z-50" style={{ background: 'transparent' }}>
      <div
        ref={popupRef}
        className={`absolute bg-background rounded-md shadow-lg ${className}`}
