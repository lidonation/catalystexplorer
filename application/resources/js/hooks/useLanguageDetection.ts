import { FRANC_TO_LOCALE_MAP, SUPPORTED_LOCALES } from '@/constants/locales';
...
const useLanguageDetection = () => {
    ...
    const detectLanguage = useCallback(
        (text: string, minLength = 50): LanguageDetectionResult => {
            ...
            try {
                ...
                const mappedLanguage =
                    (FRANC_TO_LOCALE_MAP as Record<string, string>)[detected] ||
                    'en';
    ...
    const getSuggestedLanguage = useCallback(
        (fields: Record<string, string>): string | null => {
            ...
            const languageFreq = languages.reduce(
                (acc, lang) => {
