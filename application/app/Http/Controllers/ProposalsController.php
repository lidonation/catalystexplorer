...
class ProposalsController extends Controller
{
    ...
    public function proposal(Request $request, $slug): Response
    {
        ...
        $proposalData = Cache::remember($cacheKey, now()->addMinutes(0), function () use ($proposal) {
            ...
            try {
                ...
                $data['users'] = $proposal->team ? $proposal->team->map(function ($teamMember) {
                    ...
                    // Get per-member proposal stats
                    $memberStats = $this->getTeamMemberStats($teamMember->profile_type, $profile->id);
                    ...
                    return [
                        ...
                        'proposals_count' => $memberStats['proposals_count'],
                        'open_proposals_count' => $memberStats['open_proposals_count'],
                        'funded_proposals_count' => $memberStats['funded_proposals_count'],
                        'completed_proposals_count' => $memberStats['completed_proposals_count'],
                        'proposals_by_fund' => $memberStats['proposals_by_fund'],
                    ];
                })->filter()->toArray() : [];
            } catch (\Exception $e) {
        ...
        $props = [
            ...
            'teamWithStats' => $proposalData['users'] ?? [],
    ...
    /**
     * Get proposal statistics for a single team member
     */
    private function getTeamMemberStats(string $profileType, string|int $profileId): array
    {
        try {
            // Base query for proposals this member is on
            $baseQuery = fn () => Proposal::whereHas('team', function ($query) use ($profileType, $profileId) {
                $query->where('profile_type', $profileType)
                    ->where('profile_id', $profileId);
            });

            $proposalsCount = $baseQuery()->count();
            $completedCount = $baseQuery()->where('status', 'complete')->count();
            $openCount = $baseQuery()->whereIn('status', ['pending', 'in_progress', 'onboarding'])->count();
            $fundedCount = $baseQuery()->whereNotNull('funded_at')->count();

            // Get proposals grouped by fund (most recent fund first)
            $proposalsByFund = $baseQuery()
                ->select('fund_id')
            ...
            return [
                ...
                'proposals_by_fund' => $proposalsByFund,
            ];
        } catch (\Exception $e) {
            \Log::error('Error calculating team member stats: '.$e->getMessage());
    ...
    public function generateOgImage(string $slug, Request $request)
    {
        ...
        $validated = $request->validate([
            'visibleElements' => ['sometimes', 'array'],
            'visibleElements.*' => ['string', 'in:myVote,logo,totalVotes,campaignTitle,openSourceBadge'],
            'selectedThemeId' => ['sometimes', 'string'],
    ...
    private function getOgThemeGradient(string $themeId, ?string $customColor = null): array
    {
        if ($customColor) {
            ...
            return [
                'start' => $customColor,
                'end' => $darker,
                'css' => "linear-gradient(135deg, {$customColor} 0%, {$darker} 100%)",
                'textColorClass' => $this->getTextColorClass($customColor),
            ];
        }

        $themes = [
            'navy-teal' => ['start' => '#182230', 'end' => '#475467'],
            'magenta-rose' => ['start' => '#D300EE', 'end' => '#790088'],
            'royal-blue' => ['start' => '#026BFF', 'end' => '#133C76'],
            'emerald-green' => ['start' => '#02FF28', 'end' => '#1A7613'],
            'teal-cyan' => ['start' => '#02B7FF', 'end' => '#13762A'],
            'sunset-orange' => ['start' => '#EE0000', 'end' => '#790088'],
            'lime-yellow' => ['start' => '#EED300', 'end' => '#B0950E'],
            'amber-orange' => ['start' => '#FF7802', 'end' => '#765613'],
        ];

        $theme = $themes[$themeId] ?? $themes['navy-teal'];
        $theme['css'] = "linear-gradient(135deg, {$theme['start']} 0%, {$theme['end']} 100%)";
        $primaryColor = $customColor ?? $theme['start'];
        $theme['textColorClass'] = $this->getTextColorClass($primaryColor);

        return $theme;
    }

    private function adjustColorBrightness(string $hex, int $percent): string
    {
        $hex = ltrim($hex, '#');
        $r = max(0, min(255, hexdec(substr($hex, 0, 2)) + round(2.55 * $percent)));
        $g = max(0, min(255, hexdec(substr($hex, 2, 2)) + round(2.55 * $percent)));
        $b = max(0, min(255, hexdec(substr($hex, 4, 2)) + round(2.55 * $percent)));
